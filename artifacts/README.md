...
`reduction-tests.log` records the native-only sticky reduction latch: 365 bounded states, 566,504 reference comparisons, concurrent atomic publication and failure cases. It is not guest hung-broker containment, timing, kernel-stop or formal-proof evidence. All nine guest images exclude this code. See [its observation and authority limits](../docs/REDUCTION.md).
...
`operator-tests.log` records the public-context codec and twelve [host operator workflow](../docs/OPERATOR.md) test groups. They exercise review/signing, one-shot launching, and file/failure semantics using public RFC fixtures only. No private operator identity, context, or request is shipped here. These tests are not successful real-operator provisioning or power-loss evidence.

`interactive.img` and `interactive-fixture.img` are separately compiled eight-domain release images for signed UART submission. The former requires experimental operator-mode context and refuses public RFC fixtures; the latter is explicitly labeled **PUBLIC-FIXTURE-ONLY**, never deployable. `interactive-boot.log` records actual host launch/signing, twelve commands, replay/cross-launch refusal, audit-full reductions, operator fixture refusal, and three UART breaks. `interactive-tests.log` records sanitized input/terminal/launch-administrator tests; the two `interactive*-report.txt` files record construction. `make interactive-smoke-saved` needs a native compiler for the fixture helper, but no SDK. No real credential or successful real-operator login was exercised, and host-displayed receipts are unauthenticated. See [the lifecycle and limits](../docs/OPERATOR.md).

The current interactive transcript additionally includes twelve invalid launch configurations, each with repeated NOT_READY submissions, restricted live status, and denied reads. Native logs include the expanded bootstrap/reinitialization and terminal reply/loss matrices. This coverage update changes no boot image bytes and does not establish crashed-service recovery or privileged rollback resistance.
