...
class LifecycleSystemTests(unittest.TestCase):
    def test_explicit_fixture_and_hosted_guards(self):
        ...
        for flags, accepted in (([], False), (["-DTCS_LIFECYCLE_TEST_PROFILE=1"], True),
                                (["-DTCS_LIFECYCLE_TEST_PROFILE=1", "-ffreestanding"], False),
                                (["-DTCS_LIFECYCLE_TEST_PROFILE=1", "-DTCS_DEBUG_PROFILE=1"], False)):
            with self.subTest(flags=flags):
                result = subprocess.run([*compiler, "-std=c11", "-Iinclude", "-Itests/lifecycle",
    ...
    def test_exact_profile_and_normal_exclusion(self):
        with contextlib.redirect_stdout(io.StringIO()):
            validate(ROOT / "system/lifecycle-test.system", "lifecycle-test")
        for profile, name in (("seed", "tcs"), ("terminal", "terminal"), ("isolation", "isolation"),
            ...
            with self.subTest(profile=profile):
                with self.assertRaises(ValueError): validate(ROOT / "system/lifecycle-test.system", profile)
                with self.assertRaises(ValueError): validate(ROOT / "system" / (name + ".system"), "lifecycle-test")

    def test_every_element_attribute_and_extra_authority(self):
        original = ET.parse(ROOT / "system/lifecycle-test.system").getroot()
        count = 0
            ...
            for index, element in enumerate(original.iter()):
                ...
                for name, change in changes:
                    with self.subTest(element=index, change=name):
                        ...
                        with self.assertRaises(ValueError): validate(path, "lifecycle-test")
                        count += 1
            result = subprocess.run([sys.executable, "-O", str(ROOT / "tools/check_system.py"),
                str(path), "--profile", "lifecycle-test"], capture_output=True, text=True, timeout=10)
            self.assertNotEqual(result.returncode, 0)
            self.assertIn("unexpected lifecycle-test authority", result.stderr)
        print(f"PASS lifecycle authority graph: {count} element/attribute mutations rejected")
    ...
    def test_build_is_separate_and_supervisor_has_only_model_dependency(self):
        with tempfile.TemporaryDirectory(prefix="tcs-runtime-plan-") as directory:
            result = subprocess.run(["make", "-n", "lifecycle-image", "BUILD_DIR=" + directory,
            ...
            self.assertEqual(set(links), {"controller.elf", "supervisor.elf", "broker.elf", "worker_a.elf", "worker_b.elf", "serial.elf"})
            ...
            fixture_compiles = [c for c in commands if "-c" in c and any(a.startswith("tests/lifecycle/") for a in c)]
            self.assertEqual(len(fixture_compiles), 5)
            for command in fixture_compiles:
                self.assertIn("-DTCS_LIFECYCLE_TEST_PROFILE=1", command)
