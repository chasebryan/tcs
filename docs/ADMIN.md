...
Status: a real Ed25519 request-verification core, host-tested with the policy core and cross-compiled for freestanding AArch64. It is **not linked into current boot images**. No administrator server, password login, operator key provisioning, boot-entropy service, signing CLI, or interactive mutation command is implemented by this increment. The login-method choice remains open; signed commands are the proposed host-held-key path, not a claim that the user has selected it.

This foundation separates three decisions: whether an operator signed the exact command, whether it is fresh in this administration instance, and whether policy can apply it to the intended state. A C command structure is not an unforgeable capability or proof of authentication. Future kernel-channel ownership must enforce which server may submit it to policy.
...
`tcs_policy_admin_compare_apply` is intended to run **inside the policy owner**, against a current candidate, with no intervening operation between generation comparison and candidate mutation. It requires the trusted admin actor and exact current subject generation. A stale command makes no candidate change. The existing audit/commit step then applies: audit failure blocks grants/restores, while an authorized revoke/quarantine still reduces authority. An admitted stale or audit-failed request has still consumed its administration sequence. A snapshot checked earlier in an admin server cannot replace this policy-side precondition. The new helper has no IPC adapter yet.
...
Before wiring a live admin channel, implement and test trusted boot freshness, independently authorized public-key provisioning, strict private IPC decoding, authenticated-context discovery for the host signer, definitive policy receipts, and failure/restart handling. Public RFC keys and deterministic test boot identities must never authorize deployment. Any credential creation/import requires an explicit operator action; no real credential was created in this increment.
...
`make admin-cross-check` compiles both administration files and both upstream cryptographic source files for the target. It does not link or execute an authenticated guest service. Existing QEMU profiles are regression-tested separately; passing them is not evidence of guest authentication.
