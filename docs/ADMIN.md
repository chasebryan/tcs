...
Status: a real Ed25519 request-verification core, executed in the [boot-context guest](BOOT-CONTEXT.md), a [test-only administrator/policy IPC profile](ADMIN-IPC.md), and a separate [signed interactive guest](OPERATOR.md). It is **not linked into ordinary seed/terminal/isolation images**. The experimental host workflow supports explicit identity creation, context preparation, review, signing, and one-shot launch. Interactive mutation uses only signed packets through the private administrator. Positive end-to-end tests use public fixtures; the operator profile refuses them. No real operator login, guest entropy service, durable recovery, or production provisioning is claimed. The login-method choice remains open; this is the experimental host-held-key option.
