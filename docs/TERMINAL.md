...
The smoke harness also uses [QEMU's `chardev-send-break`](https://www.qemu.org/docs/master/interop/qemu-qmp-ref.html#command-chardev-send-break) through a private local QMP socket. It first observes an echoed partial command, injects a break, requires the real driver's `INPUT LOST` report, and confirms that the suffix/delimiter cannot execute the interrupted command. A later read remains the expected next audit record. Enter and Ctrl-C recovery are both tested. This exercises a UART break error, not every overrun/framing-error schedule or software-queue overflow in the emulator; those broader cases remain future stress gates. The harness matches exact expected response bytes, so echoed prompt/fault-looking text cannot substitute for a command response. Native mocks and QEMU do not prove physical hardware behavior. A release-kernel profile still remains to remove shared debug output.
